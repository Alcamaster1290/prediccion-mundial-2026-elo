...
-- Refreshes the ELO model explainer with the v1.3 calibration parameters:
-- base_goals_per_team 1.3 -> 1.25, new elo_lambda_scale (goal-share split),
-- and the parity-aware draw_bias correction on 1X2 outcomes.
-- Apply after 25_xi_matchup_model_explainer.sql.
...
BEGIN
  ...
  RETURN (
    ...
    model_meta AS (
      SELECT
        coalesce(max(version), '1.3') AS version,
    ...
    SELECT jsonb_build_object(
      ...
      'model', jsonb_build_object(
        ...
        'probability_formula', 'ELO expected-score (elo_lambda_scale) -> Poisson; fixed 2 * base_goals_per_team expected goals split by team strength, with starter line-matchup adjustment and parity-aware draw bias',
        ...
        'notes', jsonb_build_array(
          ...
          'Las probabilidades de cada partido usan la curva ELO expected-score con elo_lambda_scale para repartir un total fijo de 2 * base_goals_per_team goles esperados.',
          ...
          'Calibracion v1.3: una correccion de empate consciente de la paridad (draw_bias x max(0, 1 - |dif| / parity_scale)) acerca la tasa de empates y la distribucion de puntos a rangos mundialistas sin destruir favoritismos.',
